...
public class GUIBarScriptkmh : MonoBehaviour {
	...
	void OnGUI()
	{
		...
		if (Background != null)
		{
            GUI.DrawTexture(new Rect(Screen.width -300, Screen.height - 150, Background.width * ScaleSize, Background.height * ScaleSize), Background);
        }

        GUI.DrawTexture(new Rect(Screen.width - 300, Screen.height - 150, ValueBar.width * ScaleSize, ValueBar.height * ScaleSize), ValueBar);

        //if you don't have a foreground texture i won't draw it
        if (Foreground != null)
		{
            GUI.DrawTexture(new Rect(Screen.width - 300, Screen.height - 150, Foreground.width * ScaleSize, Foreground.height * ScaleSize), Foreground);
        }
		...
		if (DisplayText)
		{
            ...
            TextString = speedometer.kmh.ToString("F0") + "km/h";

            GUI.Label(new Rect(Screen.width - 300 + TextOffset.x, Screen.height - 150 + TextOffset.y,ValueBar.width * ScaleSize,ValueBar.height * ScaleSize),TextString,LabelStyle);
